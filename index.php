...
startLesson('1. Введение в язык PHP для новичков');

hint('Язык PHP предназначен для создания динамических сайтов. На нём делают страницы, которые зависят от данных: 
личные кабинеты, формы, каталоги, интернет-магазины.<br>
Технически PHP нужен для генерации HTML-кода. Тексты и настройки часто хранятся в базе данных.<br>
Для PHP полезно знать базовый HTML: именно его PHP обычно генерирует.');

endLesson();


startLesson('2. Начало работы с PHP');

hint('Для запуска PHP локально нужна программа-сервер. Для Windows лучший вариант — OpenServer, для Linux — LAMP, 
для macOS — MAMP.<br>
Для первых проб можно использовать онлайн-редактор, но локальный сервер удобнее для нормальной работы.');

endLesson();


startLesson('3. Выполнение кода PHP');
...
hint('Если запустить этот файл через PHP-сервер, то сервер выполнит команды PHP и результат отправит в браузер.<br>
...
startLesson('4. Проблема с кодировками');
...
startLesson('5. Вывод ошибок PHP');

hint('PHP может показывать ошибки прямо в браузере.<br>Notice — это замечания о том, 
что в коде может что-то быть не так.<br>Warning — это предупреждения о том, что что-то пошло не так.<br>
Fatal возникает, когда PHP-код не может быть выполнен.<br>
На учебном проекте лучше включить полный вывод ошибок.');
...
startLesson('6. Полный код для запуска PHP');
...
startLesson('7. Улучшенный код для запуска');
...
startLesson('8. Обзор начальных PHP команд');
...
startLesson('9. Комментарии в PHP');

hint('Подобно языкам HTML и CSS в языке PHP можно оставлять комментарии.<br>
Однострочный комментарий начинается с <code>//</code>.');
...
startLesson('10. Комментарии в учебнике');
...
startLesson('11. Функция var_dump в PHP');
...
startLesson('13. Переменные в PHP');

hint('Основным понятием любого языка программирования является переменная. Переменная это контейнер, 
в котором можно хранить данные, например, строки или числа.<br>Каждая переменная должна иметь имя, 
которое может состоять из латинских букв, чисел и знаков подчеркивания. При этом в PHP перед переменной 
обязательно должен быть написан знак доллара <code>$</code>, а первый символ имени переменной не должен быть цифрой.');

endLesson();


startLesson('14. Использование переменных в PHP');

hint('Чтобы записать значение в переменную используется знак <code>=</code>, который называется оператором 
присваивания. например, запишем в переменную <code>$a</code> число <code>3</code>:');
...
startLesson('15. Изменения значений переменных в PHP');
...
startLesson('16. Математические операции с числами в PHP');
...
startLesson('17. Математические операции с переменными в PHP');
...
hint('Не обязательно сразу выводить результат операции, можно вначале записать его в переменную, 
...
startLesson('18. Приоритет математических операций в PHP');
...
startLesson('19. Равный приоритет в PHP');
...
startLesson('20. Группирующие скобки в PHP');

hint('При желании вы можете указывать приоритет операций через круглых скобок. например, 
переделаем наш код так, чтобы вначале выполнилось сложение, а уже потом умножение:');
...
startLesson('21. Нюансы группировки в PHP');
...
startLesson('23. Отрицательные числа в PHP');
...
startLesson('24. Остаток от деления в PHP');

hint('Существует специальный оператор <code>%</code>, через которого можно найти остаток от деления 
...
startLesson('25. Возведение в степень в PHP');

hint('Для возведения числа в степень также существует специальный оператор <code>**</code>. с его помощью 
возведем число <code>10</code> в третью степень:');
...
startLesson('26. Приоритет возведения в степень в PHP');

hint('Операция возведения в степень имеет приоритет перед умножением и делением. вначале выполнится 
возведение в степень, а затем умножение:');
...
startLesson('28. Сложение строк в PHP');
...
startLesson('29. Пробелы при сложении строк в PHP');
...
startLesson('30. Длина строки в PHP');
...
startLesson('31. Проблема с кириллицей в PHP');

hint('Функция <code>strlen</code>, как и многие другие строковые функции PHP, некорректно работает 
с кириллицей — она каждую кириллическую букву считает два раза:');
...
hint('Поэтому для строк, которые содержат или потенциально могут содержать проблемные символы, 
используйте функцию <code>mb_strlen</code> — она будет работать корректно:');
...
startLesson('32. Работа с HTML тегами в PHP');
...
startLesson('33. Атрибуты тегов в PHP');
...
startLesson('34. Логические значения в PHP');

hint('Кроме чисел и строк существует еще один тип данных — логический (boolean).<br>Логический тип данных 
используется для таких вещей, которые предполагают два варианта ответа — да или нет.<br>посмотрим на примере:');
...
startLesson('35. Значение null в PHP');

hint('В PHP существует еще одно специальное значение <code>null</code>, которое обозначает 
<code>ничего</code>.<br>посмотрим на примере:');
